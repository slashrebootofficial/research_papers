...
Entropium is a real-time, browser-based interactive demonstration that faithfully implements the complete mathematical framework of the Entropic Universe Theory (EUT). It directly executes the native pre-geometric lattice operations from EUT II (bare index set, all-to-all entropic bonds wij w_{ij} wij, Dirichlet energy minimization, dynamic local βeff \beta_{\rm eff} βeff relaxation to the rigidity attractor β=3 \beta = 3 β=3, exact spatial/residual bond partitioning with core protection). It further implements the scalar knot physics, entropic charge Q Q Q, and Rydberg-style gradient packet quantization from Stability of Scalar Knots…, the TSVF back-reaction acting exclusively on residual primordial bonds from EUT I, and the emergent 3D geometry via graph Laplacian embedding. All core phenomena — stable scalar knots with protected primordial cores, true volumetric 3D structure, localized Rydberg packets, live spectral diagnostics (coordination number, topological shell stability indicators), and TSVF bias on residual bonds — emerge directly from the equations in the papers with no ad-hoc parameters beyond visual presentation.

...
It directly executes the native pre-geometric lattice operations from *EUT II* (bare index set, all-to-all entropic bonds \(w_{ij}\), Dirichlet energy minimization, dynamic local \(\beta_{\rm eff}\) relaxation to the rigidity attractor \(\beta = 3\), exact spatial/residual bond partitioning with core protection). It further implements the scalar knot physics, entropic charge \(Q\), and Rydberg-style gradient packet quantization from *Stability of Scalar Knots and Spectral Properties of Dirichlet Minimizers on Pre-Geometric Index Sets*, the TSVF back-reaction acting exclusively on residual primordial bonds from *EUT I*, and the emergent 3D geometry via graph Laplacian embedding. All core phenomena - stable scalar knots with protected primordial cores, true volumetric 3D structure, localized Rydberg packets, live spectral diagnostics (coordination number, topological shell stability indicators), and TSVF bias on residual bonds - emerge directly from the equations in the papers with no ad-hoc parameters beyond visual presentation. \
